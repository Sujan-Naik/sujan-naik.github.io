...
export const getProjectWithPath = async (inputName: string) => {
    ...
    try {
        const directories = await fs.readdir("public/game", { withFileTypes: true });
        ...
        for (const dir of directories) {
            if (dir.isDirectory()) {
                const dirPath = path.join("public/game", dir.name);
...
export const getAllProjects = async () => {
    ...
    try {
        const fileNames = await fs.readdir("public/game/", {recursive: true});
        files = fileNames
            .filter(fileName => fileName.endsWith('.mdx'))
            .map(fileName => fileName.replace(/\.mdx$/, ''))
...
export const getDirectories = async () => {
    ...
    try {
        const files = await fs.readdir("public/game");

        // Use Promise.all to resolve the array of promises from the map
        const stats = await Promise.all(
            files.map(async (file) => {
                const filePath = `public/game/${file}`;
...
export const getFilesInDirectory = async (directory: string) => {
    let files: string[] = [];
    try {
        const dirPath = `public/game/${directory}`;
